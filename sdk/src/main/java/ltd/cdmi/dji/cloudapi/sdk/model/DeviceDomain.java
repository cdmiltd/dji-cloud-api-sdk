...
public enum DeviceDomain {
    ...
    /** value → 枚举 查找表 */
    private static final Map<Integer, DeviceDomain> BY_VALUE =
            Arrays.stream(values()).collect(Collectors.toUnmodifiableMap(
                    DeviceDomain::value, Function.identity()));
     ...
     * 按 domain 数值反查枚举。
     *
     * @param value domain 数值（0=飞行器, 2=遥控器, 3=机场）
     * @return 匹配的枚举常量
     * @throws IllegalArgumentException value 不匹配任何已知 domain
     */
    public static DeviceDomain fromValue(int value) {
        DeviceDomain domain = BY_VALUE.get(value);
        if (domain == null) {
            throw new IllegalArgumentException("未知的 domain 值: " + value);
        }
        return domain;
    }
}
