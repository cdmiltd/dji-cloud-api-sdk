...
/**
 * flighttask_execute 指令回复 output。
 *
 * <p><b>本指令无 output 字段</b>：services_reply 仅返回 {@code data.result=0} 表示成功，
 * 异步进度通过 {@code flight_task_progress} 事件上报。
 * 本 record 为占位以保证 Request/Reply 对称性，无业务字段。
 *
 * <p>若 DJI 协议后续为本指令补充 output 字段，将在此 record 扩展。
 *
 * @see FlighttaskExecuteRequest
 */
@DocUrl("https://developer.dji.com/doc/cloud-api-tutorial/cn/api-reference/dock-to-cloud/mqtt-dock.html")
@Verified(basis = "simulator WaylineTaskSimulator.handleExecute 已对接 hivemind 验证：返回 result=0 无 output，进度走 flight_task_progress 事件")
public record FlighttaskExecuteReply() {}
