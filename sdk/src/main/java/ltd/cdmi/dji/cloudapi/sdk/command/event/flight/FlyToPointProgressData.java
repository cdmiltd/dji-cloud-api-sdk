...
/**
 * fly_to_point_progress 事件 data。
 *
 * <p>对应 DJI Cloud API {@code fly_to_point_progress} 事件（events 通道）的 data。
 * 用于 flyto 指令执行结果通知，{@code need_reply=1} 需平台回复，含 fly_to_id、状态、
 * 剩余距离/时间与规划轨迹点。
 *
 * <p>对应枚举：{@link ltd.cdmi.dji.cloudapi.sdk.protocol.method.EventMethod#FLY_TO_POINT_PROGRESS}
 *
 * <p>字段依据：simulator {@code FlightCommandSimulator}（L396-L425）已对接 hivemind 验证。
 *
 * <p>{@code status} 枚举值：{@code wayline_progress}/{@code wayline_ok}。
 *
...
@Verified(basis = "simulator FlightCommandSimulator L396-L425 已对接 hivemind 验证")
public record FlyToPointProgressData(
    String flyToId,
    String status,
    Integer result,
