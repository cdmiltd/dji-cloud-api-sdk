...
public enum TopicChannel {
    ...
    /** suffix → 枚举 查找表 */
    private static final Map<String, TopicChannel> BY_SUFFIX =
            Arrays.stream(values()).collect(Collectors.toUnmodifiableMap(
                    TopicChannel::suffix, Function.identity()));
     ...
     * 按 topic 后缀反查通道类型。
     *
     * @param suffix topic 后缀，如 "osd"、"drc/up"、"property/set_reply"
     * @return 匹配的枚举常量
     * @throws IllegalArgumentException suffix 不匹配任何已知通道
     */
    public static TopicChannel fromSuffix(String suffix) {
        TopicChannel channel = BY_SUFFIX.get(suffix);
        if (channel == null) {
            throw new IllegalArgumentException("未知的 topic 后缀: " + suffix);
        }
        return channel;
    }
}
